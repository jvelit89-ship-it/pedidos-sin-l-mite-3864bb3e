        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-sidebar-foreground/60 capitalize">
              {user.role === 'vendedor' ? 'Vendedor' : user.role === 'repartidor' ? 'Repartidor' : 'Administrador'}
            </p>
          </div>
        </div>
        <button
          onClick={logout}
          className="w-full px-3 py-2 text-sm text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground rounded-lg transition-colors text-left"
        >
          Cerrar Sesión
        </button>
      </div>
