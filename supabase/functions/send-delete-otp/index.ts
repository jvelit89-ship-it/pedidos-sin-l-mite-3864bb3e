...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { orderIds, deleteAll }: SendOtpRequest = await req.json();
    ...
    // Send email with OTP
    const orderCount = deleteAll ? "TODOS" : orderIds.length.toString();
    const emailResponse = await resend.emails.send({
      from: "Sistema de Pedidos <onboarding@resend.dev>",
      to: [user.email!],
      subject: "Código de verificación para eliminar pedidos",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <h1 style="color: #dc2626;">⚠️ Confirmación de Eliminación</h1>
          <p>Has solicitado eliminar <strong>${orderCount}</strong> pedido(s).</p>
          <p>Tu código de verificación es:</p>
          <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; text-align: center; margin: 20px 0;">
            <span style="font-size: 32px; font-weight: bold; letter-spacing: 8px; color: #1f2937;">${otp}</span>
          </div>
          <p style="color: #6b7280; font-size: 14px;">Este código expira en 10 minutos.</p>
          <p style="color: #dc2626; font-size: 14px;"><strong>Advertencia:</strong> Esta acción no se puede deshacer.</p>
        </div>
      `,
    });
    ...
    console.error("Error in send-delete-otp function:", error);
    return new Response(
