...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const emailResponse = await resend.emails.send({
      ...
      to: [user.email!],
      subject: "Código de verificación para ver PIN de entrega",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <h1 style="color: #2563eb;">🔐 Verificación de Seguridad</h1>
          <p>Has solicitado ver el PIN de entrega del pedido <strong>#${orderId.slice(0, 8)}</strong>.</p>
          <p>Tu código de verificación es:</p>
          <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; text-align: center; margin: 20px 0;">
            <span style="font-size: 32px; font-weight: bold; letter-spacing: 8px; color: #1f2937;">${otp}</span>
          </div>
          <p style="color: #6b7280; font-size: 14px;">Este código expira en 10 minutos.</p>
          <p style="color: #2563eb; font-size: 14px;"><strong>Seguridad:</strong> No compartas este código con nadie.</p>
        </div>
      `,
    });
    ...
    console.error("Error in send-reveal-pin-otp function:", error);
    return new Response(
